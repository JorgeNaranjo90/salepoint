<table>
    <tr><td><h1><b>{{$purchase_order->partner->name}}</b></h1></td></tr>
    <tr><td>{{$purchase_order->partner->street .' '.$purchase_order->partner->noExt .' '.$purchase_order->partner->noInt}}</td></tr>
    <tr><td>{{$purchase_order->partner->colony .' '.$purchase_order->partner->zip .' '.$purchase_order->partner->locality}}</td></tr>
    <tr><td>{{$purchase_order->partner->country->name .' '.$purchase_order->partner->state->name .' '.$purchase_order->partner->city->name }}</td></tr>
</table>
<br>
<table>
    <tr><td></td></tr>
    <tr><td>{{$purchase_order->partner->rfc}}</td></tr>
    <tr><td>{{'SO'.$purchase_order->id}}</td></tr>
    <tr><td><h1>{{$purchase_order->status}}</h1></td></tr>
</table>
<table class="table table-striped table-hover">
    <tr>
        <th> {{trans('Image')}}</th>
        <th> {{trans('Name')}}</th>
        <th> {{trans('Qty')}}</th>
        <th> {{trans('UnitPrice')}}</th>
        <th> {{trans('Subtotal')}}</th>
    </tr>
    @foreach ($purchase_order->purchase_order_lines as $purchase_order_line)
        <tr>
            <td>
                @if(!empty($purchase_order_line->product->image))
                    <img src="data:{! mime_content_type( base64_decode($product->image)) !};base64,{{ $purchase_order_line->product->image }}" width="50px" height="50px"/>
                @else
                    <img src="data:image/png;base64," width="50px" height="50px"/>
                @endif
            </td>
            <td>{{ $purchase_order_line->name }}</td>
            <td>{{ $purchase_order_line->qty }}</td>
            <td>{{ $purchase_order_line->unitPrice }}</td>
            <td>{{ $purchase_order_line->subTotal }}</td>
        </tr>
    @endforeach
    <tr>
                    <td colspan="3"></td>
                    <td><b>Subtotal</b></td>
                    <td>{{ $purchase_order->subTotal }}</td>
                </tr>

            <tr>
                    <td colspan="3"></td>
                    <td><b>Total</b></td>
                    <td>{{ $purchase_order->total }}</td>
                </tr>

</table>
<table>
    <tr><td><img src="/images/barcode.png" width="200px" height="80px"/></td></tr>
    <tr><td>Fecha: {{$purchase_order->created_at}}</td></tr>
</table>