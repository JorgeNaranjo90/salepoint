...
@section('buttons')
    ...
    @endsection
    <a class="btn btn-info btn-sm" href="{{ route('settings.users.edit', $user->id) }}"><i class="fa fa-pencil"></i> Edit</a>
...
@section('body_page')
    <div class="col-lg-2">
        @if(!empty($user->image))
            <img src="data:{! mime_content_type( base64_decode($user->image)) !};base64,{{ $user->image }}" class="img-responsive"/>
        @else
